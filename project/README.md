...
1. **Build a global tour** to minimize travel distance (shortest-path distances using `dist`):
...
2. **Split the tour into multiple depot trips** using the *true* load-dependent cost:
...
1. **Heavy-city handling (optional)**: a small fraction of high-gold cities is served with dedicated trips, especially for larger \(\beta\), to avoid large load spikes in longer routes.

2. **Sweep ordering**: cities are ordered by polar angle around the depot; multiple angular offsets are tested (multi-start).

3. **Insertion-based route construction**:
...
2. **Strong performance for \(\beta>1\)**  
...
4. **Better scalability than full evolutionary approaches**  
...
5. **Shortest-path caching via MoveCache**  
...
1. **Runtime outliers on large \((N)\) cases, especially for \(\beta<1\)**  
...
3. **Sensitivity to internal parameters**  
...
4. **Intrinsic difficulty of \(\beta=1\)**  
